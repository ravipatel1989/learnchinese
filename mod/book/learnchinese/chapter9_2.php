...
if(isset($_GET['list']) && $_GET['list']!=""){
    ...
    $sql = "SELECT c.recordid, c.content AS contentchar, (SELECT c1.content FROM {data_content} AS c1 WHERE c1.recordid = c.recordid AND c1.fieldid = 68 AND (c1.content NOT LIKE '%$pinyin%' AND c1.content NOT LIKE '$pinyin %' AND c1.content NOT LIKE '% $pinyin')) AS pinyin FROM {data_content} AS c WHERE c.fieldid = 67 AND c.content LIKE '%$hskchar%'";
//    echo $sql; die; 
...
if(isset($_POST['hsk'])){
    ...
    foreach($hskData as $key => $value){
        foreach($sentenceData as $skey => $svalue){
            ...
            if(strpos($svalue->sentence, $value->hskchar) !== false && strpos($svalue->pinyin, $hskpinyin)===false){
             $countArr[$key]++;   
            }
        }
    }
...
<tbody>
    ...
    <tr>
        <td><?php echo $value->hskchar; ?></td><td><?php echo $value->pinyin; ?></td><td><?php echo intval($countArr[$key]); ?></td><td><a target="_blank" class="btn btn-primary" href="view.php?id=15&chapterid=69&list=<?php echo $value->recordid; ?>">View list</a></td>
    </tr>
    <?php } ?>
</tbody>
</table>
