...
$pinyinData = $DB->get_records_sql("SELECT r.id, content FROM {data_content} AS c LEFT JOIN {data_records} AS r ON c.recordid = r.id WHERE c.recordid IN (SELECT recordid FROM {data_content} WHERE fieldid = 29 AND content = '') AND c.fieldid = '28' AND r.dataid = 3");
$pinyinValues = [];
        ...
        foreach($finalArr as $key => $value){
            echo '<tr>';
                ...
                for($i=0;$i<24;$i++){
                    ...
                    $tonecnt = 0;
                    if($pinyincounts[$pinyinchar.'1'] > 0){
                        $tonecnt++;
                    }
                    if($pinyincounts[$pinyinchar.'2'] > 0){
                        $tonecnt++;
                    }
                    if($pinyincounts[$pinyinchar.'3'] > 0){
                        $tonecnt++;
                    }
                    if($pinyincounts[$pinyinchar.'4'] > 0){
                        $tonecnt++;
                    }
                    if($pinyincounts[$pinyinchar.'5'] > 0){
                        $tonecnt++;
                    }
                    if(intval($tonecnt > 2)){
                        echo '<td><a href="'.$CFG->wwwroot.'/mod/book/view.php?id=15&chapterid=22&search='.$pinyinchar.'"><i class="fa fa-play"></i></a></td>';
                    }else{
                        echo '<td>X</td>';
                    }
                }
