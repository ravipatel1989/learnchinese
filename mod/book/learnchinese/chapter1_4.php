...
$pinyinData = $DB->get_records_sql("SELECT r.id, c.content, (select cc.content FROM {data_content} as cc WHERE cc.fieldid = 14 AND cc.recordid = c.recordid) AS charcontent FROM {data_content} AS c LEFT JOIN {data_records} AS r ON c.recordid = r.id WHERE c.fieldid = '28' AND r.dataid = 3");
            ...
            echo '<tr>';
                ...
                for($i=0;$i<24;$i++){
                    ...
                    if(intval($pinyincounts[$pinyinchar] > 0)){
                        echo '<td><a href="javascript:void(0);" class="pinyinchar" data-ajaxurl="'.$ajaxAction.'" data-pinyin="'.$pinyinchar.'" data-action="get_pinyin_with_tone" data-toggle="modal" data-target="#pinyinModal">'.$pinyincounts[$pinyinchar].'</a></td>';
                    }else{
                        echo '<td>X</td>';
                    }
                }
