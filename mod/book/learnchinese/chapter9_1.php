...
if(isset($_GET['list']) && $_GET['list']!=""){
    ...
    $sql = "SELECT c.recordid, c.content AS contentchar, (SELECT c1.content FROM {data_content} AS c1 WHERE c1.recordid = c.recordid AND c1.fieldid = 68 AND (c1.content LIKE '%$pinyin%' AND c1.content NOT LIKE '% $pinyin %' AND c1.content NOT LIKE '$pinyin %' AND c1.content NOT LIKE '% $pinyin')) AS pinyin FROM {data_content} AS c WHERE c.fieldid = 67 AND c.content LIKE '%$hskchar%'";
...
//    $sql = "SELECT c.recordid, c.content AS hskcharacter, (SELECT c3.content FROM {data_content} AS c3 WHERE c3.fieldid = 49 AND c3.recordid = c.recordid) AS hskpinyin, (SELECT count(c2.id) FROM {data_content} AS c2 WHERE (c2.fieldid = 67 AND c2.content) AND c2.content LIKE CONCAT('%',c.content ,'%')) AS countchar FROM {data_content} as c ORDER BY `countchar` DESC";
    ...
    foreach($hskData as $key => $value){
        foreach($sentenceData as $skey => $svalue){
            ...
            if(strpos($svalue->sentence, $value->hskchar) !== false && !in_array($hskpinyin, $pinyinArr) && strpos($svalue->pinyin, $hskpinyin)!==false){
             $countArr[$key]++;   
            }
        }
    }
...
<tbody>
    ...
    <tr>
        <td><?php echo $value->hskchar; ?></td><td><?php echo $value->pinyin; ?></td><td><?php echo intval($countArr[$key]); ?></td><td><a target="_blank" class="btn btn-primary" href="view.php?id=15&chapterid=67&list=<?php echo $value->recordid; ?>">View list</a></td>
    </tr>
    <?php } ?>
</tbody>
</table>
