            ...
            echo '<tr>';
                ...
                for($i=0;$i<24;$i++){
                    ...
                    if(!empty($pinyincounts[$arrval]) && intval($pinyincounts[$arrval] > 0)){
                        echo '<td><a href="'.$CFG->wwwroot.'/mod/book/view.php?id=15&chapterid=14&search='.$arrval.'">'.$pinyincounts[$arrval].'</a></td>';
                    }else{
                        echo '<td>X</td>';
                    }
                }
...
<?php }else{
    ...
    echo '<a href="'.$_SERVER['HTTP_REFERER'].'" class="btn btn-info" role="button" style="margin-bottom:5px;">Go back</a>';
    echo '</div>';
    echo '<div class="clear"></div>';
    ...
    foreach($pinyinData as $key => $value){
        if(strpos($value->pinyin,$searchVal) !== false){
            ...
            if(is_siteadmin()){
                ...
                echo '<td><a href="javascript:void(0);" class="btn btn-primary editpinyin" data-ajaxurl="'.$ajaxAction.'" data-id="'.$value->id.'" role="button">Edit</a></td>';
            }
            echo '</tr>';
        }
    }
